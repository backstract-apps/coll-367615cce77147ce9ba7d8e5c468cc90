...
async def post_users(db: Session, raw_data: schemas.PostUsers):
    ...
    query = query.join(test, and_(models.Users.role_id == models.Roles.role_id))

    test1 = query.first()
    test1 = (
        [
            {
                "test1_1": s1.to_dict() if hasattr(s1, "to_dict") else vars(s1),
                "test1_2": s2.to_dict() if hasattr(s2, "to_dict") else vars(s2),
            }
            for s1, s2 in test1
        ]
        if test1
...
async def post_roles(db: Session, raw_data: schemas.PostRoles):
    ...
    post_get_records = (
        [
            ...
            for s1, s2 in post_get_records
        ]
        if post_get_records
        else post_get_records
    )
    res = {
        "roles_inserted_record": roles_inserted_record,
        "post_get_records": post_get_records,
    }
    return res
